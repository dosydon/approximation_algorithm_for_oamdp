...
pub trait StatesActions {
    type State: Eq + PartialEq + Debug + Copy + Clone + Hash;
    type Action: Eq + PartialEq + Debug + Copy + Clone + Hash;
}
...
pub fn to_var_name<S: ToVarName, A: Debug>(s: &S, a: &A) -> String {
    format!("{}_{:?}", s.to_var_name(), a)
}
