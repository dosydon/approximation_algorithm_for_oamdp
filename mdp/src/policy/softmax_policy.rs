...
use super::policy_traits::GetActionProbabilityMut;
...
impl<M: ActionAvailability + StateEnumerable + ActionEnumerable + PMass<f32> + Cost>
    GetActionProbability<M::Action, M> for SoftmaxPolicy<ValueTable<M::State>>
{
    fn get_action_probability(&self, s: &M::State, a: &M::Action, mdp: &M) -> f32 {
...
impl<M: ActionAvailability + StateEnumerable + ActionEnumerable + PMass<f32> + Cost>
    GetActionProbabilityMut<M::Action, M> for SoftmaxPolicy<ValueTable<M::State>>
{
    fn get_action_probability_mut(
        ...
        self.get_action_probability(s, a, mdp)
    }
}
...
impl<M: ActionEnumerable + StateEnumerable + PMass<f32> + Rsa + DiscountFactor>
    GetActionProbability<M::Action, M> for SoftmaxRewardPolicy<M>
{
    fn get_action_probability(&self, s: &M::State, a: &M::Action, mdp: &M) -> f32 {
...
mod tests {
    ...
    fn test_grid_world_softmax() {
        ...
        assert_approx_eq!(
            0.40183,
            softmax_policy.get_action_probability(&GridWorldState::new(0, 0), &AttemptRight, &mdp,),
            err
        );
        assert_approx_eq!(
            0.287654,
            softmax_policy.get_action_probability(&GridWorldState::new(0, 0), &AttemptDown, &mdp,),
            err
        );
        assert_approx_eq!(
            0.15698,
            softmax_policy.get_action_probability(&GridWorldState::new(0, 0), &AttemptUp, &mdp,),
            err
        );
        assert_approx_eq!(
            0.1535276,
            softmax_policy.get_action_probability(&GridWorldState::new(0, 0), &AttemptLeft, &mdp,),
            err
        );
    }
}
