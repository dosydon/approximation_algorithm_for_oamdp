...
impl<M: GetNextStateMut> GetNextStateMut for CacheWrapper<M> {
    fn get_next_state_mut(
        ...
        self.mdp.get_next_state_mut(s, a, rng)
    }
}
...
// impl<M: PMassMut<f64>> PMassMut<f64> for CacheWrapper<M>
// where
//     M: PMassMut<f64, Distribution = Vec<(Self::State, f64)>>,
// {
//     type Distribution = Vec<(Self::State, f64)>;
//     fn p_mass_mut(&mut self, s: &Self::State, a: &Self::Action) -> Vec<(Self::State, f64)> {
//         if let Some(v) = self.p_mass_cache_64.get(&(*s, *a)) {
//             v.clone()
//         } else {
//             let v = PMassMut::<f64>::p_mass_mut(&self.mdp, s, a);
//             self.p_mass_cache_64.insert((*s, *a), v.clone());
//             v
//         }
//     }
