...
use mdp::baker_grid::baker_grid_reset::BakerGridResetBuilder;
use mdp::baker_grid::baker_grid_reset::BakerGridResetMDP;
use mdp::baker_grid::*;
...
fn main() {
    ...
    let possible_goals = vec![BakerGridState::new(0, 4), BakerGridState::new(2, 4)];
    let partial_mdp = BakerGridPartialMDP::new(5, 5, vec![])
        .set_initial_state(Coordinate2::new(2, 0))
        .set_prob_veering(0.3);
    //     let pair = baker_factory("Tiny2");
    //     let partial_mdp = pair.0;
    let partial_mdp = BakerGridResetBuilder::new(partial_mdp, 0.1, vec![]);
    let goals = [possible_goals[0], possible_goals[1]];
    println!("{:?}", goals);

    let softmax_policy = SoftmaxPolicyBuilder::new(0.3);
    let mut oamdp =
        OAMDP2::<SoftmaxModel<BakerGridResetMDP, 2>, BakerGridResetMDP>::new_implicit_model(
            &partial_mdp,
            &softmax_policy,
            goals,
            1,
            belief_cost_function::BeliefCostType::Disimulation,
            Objective::LinearCombination(2.0, 0.1),
    ...
    let grid = oamdp.mdp.mdp.grid2d.clone();
