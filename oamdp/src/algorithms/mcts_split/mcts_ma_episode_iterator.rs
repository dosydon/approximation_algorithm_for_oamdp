...
pub struct MCTSMAEpisodeIterator<'a, M: StatesActions + DomainAction + Message, P> {
    mcts: &'a mut MCTSMA<M, P>,
    node_id: usize,
    rng: &'a mut rand::rngs::ThreadRng,
}

impl<'a, M: StatesActions + InitialState + DomainAction + Message, P>
    MCTSMAEpisodeIterator<'a, M, P>
{
    pub fn from_initial_state(
        mcts: &'a mut MCTSMA<M, P>,
        rng: &'a mut rand::rngs::ThreadRng,
    ) -> Self {
        MCTSMAEpisodeIterator {
            mcts,
            node_id: 0,
            rng,
        }
...
impl<
    ...
    > Iterator for MCTSMAEpisodeIterator<'a, M, P>
where
    M::Action: From<(M::DomainAction, M::Message)> + Set<M::Message>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let a = if let Some((m_id, a_id)) = pair {
            debug!("a_id: {}", a_id);
            debug!("m_id: {}", m_id);
            let m = self.mcts.arena.get_node(self.node_id).children[m_id].assoc;
            let a = self.mcts.arena.get_node(self.node_id).children[m_id].children[a_id].a;
            M::Action::from((a, m))
        } else {
            self.mcts
                .base_line_policy
        ...
        if let Some((m_id, a_id)) = pair {
            if let Some(ss_id) = self.mcts.find_s(self.node_id, m_id, a_id, ss) {
                info!("node_id: {:?}", ss_id);
                self.node_id = ss_id;
            } else {
                warn!("creating a new node");
